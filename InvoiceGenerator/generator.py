...
Variabilní symbol: %s"""
    ...
    def drawItems(self, TOP, LEFT):
        ...
        for x in self.items:
            ...
            self.pdf.drawString(
                (LEFT + 122) * mm, (TOP - i) * mm, "%.2f,- kč" % x.price
            )
            self.pdf.drawString(
                (LEFT + 150) * mm, (TOP - i) * mm, "%.2f,- kč" % (x.total())
            )
            i += 5
            total += x.total()
        ...
        self.pdf.setFont("DejaVu", 12)
        self.pdf.drawString(
            (LEFT + 130) * mm, (TOP - i - 10) * mm, "Celkem: %d ,- kč" % total
        )
    ...
    def drawDates(self, TOP, LEFT):
        ...
        self.pdf.drawString(
            (LEFT) * mm,
            (TOP + 1) * mm,
            "Datum vystavení: %s" % today.strftime("%d.%m.%Y"),
        )
        self.pdf.drawString(
            (LEFT) * mm,
            (TOP - 4) * mm,
            "Datum splatnosti: %s" % payback.strftime("%d.%m.%Y"),
        )
        self.pdf.drawString(
